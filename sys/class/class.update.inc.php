...
class update extends db_connect
{
    ...
    function addColumnToUsersTable47()
    {
        $stmt = $this->db->prepare("ALTER TABLE users ADD admob_create_at INT(10) UNSIGNED DEFAULT 0 after admob");
        $stmt->execute();
    }

    function addColumnToUsersTable48()
    {
        $stmt = $this->db->prepare("ALTER TABLE users ADD ghost_create_at INT(10) UNSIGNED DEFAULT 0 after ghost");
        $stmt->execute();
    }

    // For version 7.0

    function addColumnToLikesTable1()
    {
        $stmt = $this->db->prepare("ALTER TABLE likes ADD likeType INT(11) UNSIGNED NOT NULL DEFAULT 0 after itemId");
        $stmt->execute();
    }

    function addColumnToGalleryLikesTable1()
    {
        $stmt = $this->db->prepare("ALTER TABLE gallery_likes ADD likeType INT(11) UNSIGNED NOT NULL DEFAULT 0 after itemId");
        $stmt->execute();
    }

    function addColumnToNotificationsTable1()
    {
        $stmt = $this->db->prepare("ALTER TABLE notifications ADD notifySubType INT(11) UNSIGNED NOT NULL DEFAULT 0 after notifyType");
        $stmt->execute();
    }

    // For version 7.1

    function addColumnToUsersTable49()
    {
        $stmt = $this->db->prepare("ALTER TABLE users ADD ap_id varchar(50) DEFAULT '' after tw_id");
        $stmt->execute();
    }

    // For version 7.2

    function addColumnToUsersTable50()
    {
        $stmt = $this->db->prepare("ALTER TABLE users ADD account_free INT(10) UNSIGNED DEFAULT 0 after state");
        $stmt->execute();
    }

    //

    public function updateUsersTable48()
    {
    ...
    function addColumnToUsersTable51()
    {
        $stmt = $this->db->prepare("ALTER TABLE users ADD allowVideoCalls INT(6) UNSIGNED DEFAULT 1 after allowShowMyGifts");
        $stmt->execute();
    }
