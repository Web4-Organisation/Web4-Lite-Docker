...
class comments extends db_connect
{
    ...
    {
        $sql = "SELECT count(*) FROM ".$this->table." WHERE removeAt = 0";
    ...
    public function getStream($itemId = 0, $language = 'en')
    {
        if ($itemId == 0) {
        ...
        $sql = "SELECT id FROM $this->table WHERE id < (:itemId) AND removeAt = 0 ORDER BY id DESC LIMIT 20";
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->rowCount() > 0) {
                ...
                while ($row = $stmt->fetch()) {
                    ...
                    array_push($result['items'], $msgInfo);
