...
class post extends db_connect
{
    ...
    public function recalculate($postId) {
        ...
        $reactions = new reactions($this->db);
        $reactions->setRequestFrom($this->getRequestFrom());
        $likes_count = $reactions->count($postId);
        unset($reactions);
        ...
        $rating = $likes_count['count'] + $comments_count + $reposts_count;
        ...
        $stmt->bindParam(":likesCount", $likes_count['count'], PDO::PARAM_INT);
    ...
    public function quikInfo($row)
    {
        ...
        if ($this->getRequestFrom() != 0) {
            ...
            if ($row['likesCount'] != 0) {
                ...
                $reactions_result = $reactions->is_exists($row['id'], $this->getRequestFrom());

                if ($reactions_result['exists']) {

                    $myLike = true;
                    $myLikeType = $reactions_result['type'];
                }
