...
class notify extends db_connect
{
    ...
    public function setNotifySubType($notifyId, $subType = 0)
    {
        $stmt = $this->db->prepare("UPDATE notifications SET notifySubType = (:subType) WHERE id = (:notifyId)");
        $stmt->bindParam(":notifyId", $notifyId, PDO::PARAM_INT);
        $stmt->bindParam(":subType", $subType, PDO::PARAM_INT);
        $stmt->execute();
    }
